// src/plugins/cu-tokens/vitepress.ts — Traduce los tokens de diseño de ComegenUI
// (fuente de verdad: `comegen.config.json`) al formato de tema de VitePress.
//
...
//
// Es código de build (lo consume `cli/vitepress.mjs`), no entra en el bundle UMD.
import { generateThemesCSS } from './css'
import { generateShikiThemes } from './shiki'
import { DEFAULTS, DEFAULT_OPACITIES, extractColors, extractShared } from './defaults'

/** CSS de los temas CU con selectores compatibles con VitePress. */
export function buildThemesCss(cuConfig: Record<string, any> = {}) {
 ...
 */
export function buildShikiThemes(cuConfig: Record<string, any> = {}) {
...
export function buildVitePressBridgeCss() {
  return `/* Generado por cu-tokens: puente VitePress → tokens de ComegenUI. */
html:root {
