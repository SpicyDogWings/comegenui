...
//   node src/plugins/khadgar-docs/cli.mjs --scaffold   # crea el .doc.json base faltante
...
import { renderDoc, vueUsageBlock } from "./render.mjs";

function parseArgs(argv) {
  const args = { check: false, scaffold: false, only: null, config: null };
  for (let i = 0; i < argv.length; i++) {
    ...
    if (arg === "--check") args.check = true;
      ...
      console.log("khadgar-docs [--check] [--scaffold] [--only A,B] [--config <file>]");
      process.exit(0);
    }
  }
  return args;
...
/** Sidecar base para un componente sin `.doc.json`: arranca con la sección de uso. */
function buildScaffold(component) {
  const section = component.customElement
    ? {
        title: "Uso en HTML plano",
        titleVue: "Uso en Vue",
        body: "```html\n<!-- TODO: ejemplo vanilla -->\n```",
        bodyVue: vueUsageBlock(component),
      }
    : { title: "Uso en Vue", bodyVue: vueUsageBlock(component) };
  return {
    props: {},
    events: {},
    slots: {},
...
// --scaffold: crea el sidecar base de los componentes que no tienen uno. No pisa
// los existentes ni genera fichas.
if (args.scaffold) {
  let created = 0;
  for (const component of index.components) {
    const key = component.tag ?? component.slug;
    const file = resolve(docsDir, `${key}.doc.json`);
    if (existsSync(file)) continue;
    mkdirSync(docsDir, { recursive: true });
    writeFileSync(file, `${JSON.stringify(buildScaffold(component), null, 2)}\n`);
    console.log(`  + ${key}.doc.json`);
    created++;
  }
  console.log(`khadgar-docs: ${created} sidecar(s) base creados en ${docsDir}`);
  process.exit(0);
}
