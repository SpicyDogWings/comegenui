...
> **Regla de oro #3 — Workflow de ramas (activa en CADA tarea de desarrollo):** creá una rama nueva **solo cuando la rama base es `main`** y la tarea es una feature/fix/docs/tests con entidad propia (ej: `feat/mi-feature`, `fix/arreglo-x`, `docs/tema-y`, `test/componente-z`). **Si ya estás en una rama de trabajo (base ≠ `main`), NO crees otra rama** salvo que el usuario lo pida explícitamente: trabajá sobre la rama actual. `main` solo recibe merges (idealmente con MR/review); al terminar, mergear la rama en `main` (o abrir el MR) y borrar la rama local.
