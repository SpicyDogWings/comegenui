...
| Páginas legacy sin tabs (hoy: ninguna — playground completo; verificar con el comando de abajo al tocar cualquier página) | Si una sección vieja NO usa `SectionDemo`, migrarla al tocarla (con Vanilla solo si el componente está en lib) — no dejar secciones híbridas. La lista se re-deriva con el comando de auditoría de abajo. |
