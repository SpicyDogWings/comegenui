...
| 3 | **Story** | `src/stories/{category}/X.stories.ts`: generala con `pnpm khadgar:generate X` (desde el contrato del `.vue`) y refinala con `X.stories.config.json`; `checks.l1` (y `ce`/`umd` cuando aplique). | [`04`](04-stories-y-tests.md) |
...
| 5 | **Metadata + extras** | `pnpm khadgar:generate X --meta-only` (`tokens`/`api` → Style/API en la página genérica). Extra **Programmatic** solo si el componente usa `defineExpose`; **Events** si emite eventos. | [`04`](04-stories-y-tests.md) · [`05`](05-playground.md) |
| 6 | **Playground** | **Automático**: el plugin `khadgar` registra la ruta y el nav desde la story. Página genérica en `/playground/components/:name`; página física opcional con `--pages`. | [`05`](05-playground.md) |
| 7 | **Docs** | Ficha generada con `pnpm khadgar:generate <X> --docs` desde el SFC de la lib (+ sidecar `cu-x.doc.json` con la prosa curada) y su entrada en los índices. | [`07`](07-documentacion.md) + skill `comegen-ui-docs` |
...
- [ ] **Docs**: ficha `docs/skills/use-comegen/componentes/cu-x.md` **generada** con `pnpm khadgar:generate <X> --docs` (fuente de verdad = el componente) + sidecar `cu-x.doc.json` con la prosa curada.
...
- **Plugin** (`src/plugins/khadgar/`): runtime (Vue plugin + nav automático), `generate` (contrato-driven) y reporter de vitest (genérico/reutilizable; ver su `README.md`).
...
| `pnpm khadgar:generate <X>` | Genera la story **desde el contrato** del `.vue` (props/emits/exposes/slots/tokens) → secciones + snippets + checks + `api`. Lee `X.stories.config.json`. No pisa la story si existe. |
| `pnpm khadgar:generate <X> --meta-only` | Actualiza solo `tokens`/`api` de una story existente sin tocar secciones/checks/extras; preserva descripciones curadas. |
| `pnpm khadgar:generate --all` | Barre `componentsDir` y genera/actualiza todas las stories (metadata). |
| `pnpm khadgar:generate <X> --pages` | Además emite una página física editable en `playgroundDir` (override de la genérica). |
| `pnpm khadgar:generate <X> --docs` | Genera/actualiza la ficha `componentes/cu-x.md` desde el SFC que distribuye la lib (`.ce.vue` o `.vue`); prosa curada en el sidecar `cu-x.doc.json`. `--all --docs` para todas; `--docs --check` detecta drift (lo corre el preflight). |
...
| Tipos de stories / runner | Contrato `src/plugins/khadgar/contract.ts` · runner `src/plugins/khadgar/tests/runner.l1.ts` (`src/stories/types.ts` y `runner.l1.ts` son shims backwards-compat) |
| Playground (genérico) | Plugin `src/plugins/khadgar/` (runtime en `runtime/`, chrome fallback en `runtime/chrome/`, nav automático) |
| Página física (opcional) | `src/playground/X.vue` (override; la crea `--pages`) |
| Config del playground | `khadgar.config.json` (raíz) |
